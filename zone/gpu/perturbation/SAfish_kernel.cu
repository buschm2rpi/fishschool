...
{
	/* divided by 2^32 */
	return ((((float)mt19937s()) + 0.5)*(1.0/4294967296.0)); 
}

//kernel for device functionality
//each multiprocessor work on one realization
//BLOCKSZIE threads of each multiprocessor will cooperate with 
//each other through shared memory to finish one realization
//Difrerents blocks will run different realizations to get ensembles
...
//@dDy:
//@preseed : random seed for each threads

__global__ 
void fishKernel( float * dPx, float * dPy, float * dVx, float * dVy, 
	float* dDx, float* dDy, float * preseed, float hRatio, float* averageVx, float* averageVy) 
{
	float Rx=0, Ry=0, Rz=0, sum=0;
	float rr=1.0, drp=6.0, s=1.0, beta;
	float theta=115.0, tau=0.2, ratio = hRatio;//SETRATIO;
	...
	mt19937si(preseed[bid *BLOCKSIZE + tid]);
	
	int steps=0;
	while(steps<MAXSTEPSPERITER) {
		...
		{	
			...
			for (int j = 0; j < FISHNUM; j++) {
				cPx[0] += dVx[bid*FISHNUM+j];
				cPy[0] += dVy[bid*FISHNUM+j];
				//cPx[0] += cDx[j];
				//	hostPy[i] += hostVy[i*100+j];
				//printf("Vx %f tVx %f \n", dVx[bid*FISHNUM+j],cPx[0]);
			}
			averageVx[bid*MAXSTEPSPERITER+steps] = cPx[0]/FISHNUM;
			averageVy[bid*MAXSTEPSPERITER+steps] = cPy[0]/FISHNUM;
			//printf("Block %d, step %d average :%f  ", bid, steps,averageVx[bid*MAXSTEPSPERITER+steps]);
		}
		__syncthreads();
